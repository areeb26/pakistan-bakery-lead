...
async def run(
    ...
    async with async_playwright() as p:
        ...
        print(f"\n{'=' * 60}\nSCRAPING {query} (target: {limit})\n{'=' * 60}", flush=True)
        collected: list[dict] = []
        seen: set[str] = set()

        needed = limit - len(collected)
        try:
            batch = await scrape_query(page, query, needed, seen, "custom")
            collected.extend(batch)
        except Exception as e:
            print(f"  QUERY FAILED: {e}", flush=True)

        results["custom"] = collected[:limit]
        save_outputs(collected[:limit], "custom", output_dir)
...
def save_outputs(leads: list[dict], lead_type: str, output_dir: str) -> tuple[str, str]:
    ...
    base = Path(output_dir) / f"{lead_type}_{ts}"
    # also keep a stable latest name
    latest_json = Path(output_dir) / f"{lead_type}.json"
    latest_csv = Path(output_dir) / f"{lead_type}.csv"
...
def parse_args():
    p = argparse.ArgumentParser(description="Scrape custom query from Google Maps")
    p.add_argument(
        "--query",
        required=True,
    ...
    p.add_argument(
        "--test",
        action="store_true",
        help="Quick test mode: 5 leads, headless",
    )
    return p.parse_args()
...
def main():
    ...
    print(
        f"Custom Maps scraper | query='{args.query}' | limit={limit} | headless={headless}",
        flush=True,
    )
    ...
    print(f"  Leads: {len(leads)} | phone={with_phone} | website={with_web}")
