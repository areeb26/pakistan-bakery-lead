...
QUERIES = {
    "travel": [
        ...
        "tour operators Pakistan",
        "hajj umrah travel agency Pakistan",
        "travel agency Karachi",
        "travel agency Lahore",
        "travel agency Islamabad",
        "visa consultants Pakistan",
        "travel and tours Pakistan",
        "domestic tour operators Pakistan",
        "air travel agency Pakistan"
    ],
}
...
def save_outputs(leads: list[dict], lead_type: str, output_dir: str) -> tuple[str, str]:
    ...
    base = Path(output_dir) / f"pakistan_{lead_type}_{ts}"
    # also keep a stable latest name
    latest_json = Path(output_dir) / f"pakistan_{lead_type}.json"
    latest_csv = Path(output_dir) / f"pakistan_{lead_type}.csv"
...
def parse_args():
    p = argparse.ArgumentParser(description="Scrape Pakistan travel agencies from Google Maps")
    p.add_argument(
        "--type",
        choices=["travel"],
        default="travel",
        help="What to scrape (default: travel)",
    )
...
def main():
    ...
    print(
        f"Pakistan Travel Maps scraper | types={types} | limit={limit} | headless={headless}",
        flush=True,
    )
